...
const Inicio = ({ navigation }) => {
  ...
  const getData = async () => {
    ...
    if (lista === undefined || lista === null) {
      await OroPuro.get(
        `/*?slpName=${user.SlpName}&rutCode=${user.RutCode}`
      )
        .then(result => {
          ...
          AsyncStorage.setItem("ListInv", JSON.stringify(result.data.Productos));
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#ab000d" }}>
      <ScrollView style={styles.container}>
        <HeaderOP title="Inicio" onPress={() => navigation.toggleDrawer()} />
